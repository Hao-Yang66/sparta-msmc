...
struct CommMacro {
    double v[3];
    double Temp;            // MSMC convention: total/equilibrium temperature Teq
};
struct NoCommMacro {
    int init_flag;
    int do_relaxation;
    ...
    double sum_erot;
    double sum_evib;
    double sum_erot_vi[3];
    double sum_evib_vi[3];
    double rho;   // mass density
    double ptr;   // translational pressure, time-averaged
    double peq;   // equilibrium pressure based on Teq, time-averaged
    double Ttr;
    double Trot;
    double Tvib;
    double qrot[3];

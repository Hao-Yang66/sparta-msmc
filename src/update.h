...
class Update : protected Pointers {
 public:
  ...
  // args for accurate move of particles if move_surf,
  // change only if fix move/surf is used.
  struct UpdateMoveSurf {
      int flag;
      class FixMoveSurf* fixmovesurf;
      class MoveSurf* movesurf;
      double rotate_o[3];
      double rotate_omega; // rad/sec
  };
  UpdateMoveSurf ms;
  ...
  void caculate_rl(double* p1, double* p2, RotateLine& rl);

  double caculate_yP(double* v, double* x, RotateLine& rl, double t, double dt_done);
  double caculate_xP(double* v, double* x, RotateLine& rl, double t, double dt_done);

  double secand_method(double* v, double* x, RotateLine& rl, double t1, double t2, double dt_done);
  double secand_dichotomy_method(double* v, double* x, RotateLine& rl, double t1, double t2, double dt_done);

  double rotate_collide_time(double* v, double* x, RotateLine& rl, double dtmax, double dt_done, double* point);

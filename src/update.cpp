...
void Update::run(int nsteps)
{
  ...
  for (int i = 0; i < nsteps; i++) {
    ...
    (this->*moveptr)();
    if (ms.flag) ms.fixmovesurf->mid_of_step();
...
/* ---------------------------------------------------------------------- */
void Update::caculate_rl(double* p1, double* p2, RotateLine& rl)
{
    double x1 = p1[0] - ms.rotate_o[0], y1 = p1[1] - ms.rotate_o[1],
        x2 = p2[0] - ms.rotate_o[0], y2 = p2[1] - ms.rotate_o[1];

    // o'点坐标
    double x = (y2 - y1) * (x1 * y2 - x2 * y1) 
        / ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));    
    double y = (x1 - x2) * (x1 * y2 - x2 * y1)
        / ((x2 - x1) * (x2 - x1) + (y2 - y1) * (y2 - y1));

    // o'x' o'y'
    double xx = x2 - x1, yy = y2 - y1;
    rl.ox[0] = xx / sqrt(xx * xx + yy * yy);
    rl.ox[1] = yy / sqrt(xx * xx + yy * yy);
    rl.oy[0] = -rl.ox[1]; rl.oy[1] = rl.ox[0];
    
    // r & theta
    rl.r = sqrt(x * x + y * y);
    rl.theta0 = atan2(y2 - y1, x2 - x1) + MathConst::MY_PI / 2;
    rl.x_A = sqrt((x1 - x) * (x1 - x) + (y1 - y) * (y1 - y));
    rl.x_B = sqrt((x2 - x) * (x2 - x) + (y2 - y) * (y2 - y));
    if ((x1 - x) * (x2 - x1) + (y1 - y) * (y2 - y1) < 0) rl.x_A = -rl.x_A; //x'轴沿A->B方向
    if ((x2 - x) * (x2 - x1) + (y2 - y) * (y2 - y1) < 0) rl.x_B = -rl.x_B;

    rl.reverse_flag = false;
    if (rl.r > 0 && x * rl.oy[0] + y * rl.oy[1] < 0) {
        rl.reverse_flag = true;
        rl.x_A *= -1;rl.x_B *= -1;
        rl.theta0 += MathConst::MY_PI;
        rl.ox[0] *= -1; rl.ox[1] *= -1;
        rl.oy[0] *= -1; rl.oy[1] *= -1;
    }

    //printf("\n caculate_rl: x: %g, y:%g, p1: %g %g, p2: %g %g, theta0: %g r: %g A: %g B: %g\n",
    //    x, y, x1, y1, x2, y2, rl.theta0, rl.r, rl.x_A, rl.x_B);
    return;
}

/* ---------------------------------------------------------------------- */
double Update::caculate_yP(double* v, double* x, RotateLine& rl, double t, double dt_done)
{
    double xt = x[0] - ms.rotate_o[0] + t * v[0];
    double yt = x[1] - ms.rotate_o[1] + t * v[1];
    double theta_t = rl.theta0 + ms.rotate_omega * (t + dt_done);
    return (xt - rl.r * cos(theta_t)) * cos(theta_t)
        + (yt - rl.r * sin(theta_t)) * sin(theta_t);
}

/* ---------------------------------------------------------------------- */
double Update::caculate_xP(double* v, double* x, RotateLine& rl, double t, double dt_done)
{
    double xt = x[0] - ms.rotate_o[0] + t * v[0];
    double yt = x[1] - ms.rotate_o[1] + t * v[1];
    double theta_t = rl.theta0 + ms.rotate_omega * (t + dt_done);
    return (xt - rl.r * cos(theta_t)) * sin(theta_t)
        - (yt - rl.r * sin(theta_t)) * cos(theta_t);
}

/* ---------------------------------------------------------------------- */

double Update::secand_dichotomy_method(double* v, double* x, RotateLine& rl, double t1, double t2, double dt_done)
{
    const double tolerance = 1e-10 * MIN(domain->boxhi[0] - domain->boxlo[0],
        domain->boxhi[1] - domain->boxlo[1]); // need further test
    const double tolerance_t = 1e-10 * update->dt;
    const int max_iter = 100; // need further test
    double f1 = caculate_yP(v, x, rl, t1, dt_done);
    double f2 = caculate_yP(v, x, rl, t2, dt_done);
    
    if (fabs(f1) < tolerance) return t1;
    if (fabs(f2) < tolerance) return t2;
    ...
    for (int i = 0; i < max_iter; ++i) {
        if (fabs(f1 - f2) < 1e-50) { 
            error->warning(FLERR, "Division by near-zero difference in secant method.");
            return t2;
        }
        double t_new = t2 - f2 * (t2 - t1) / (f2 - f1);
        double f_new = caculate_yP(v, x, rl, t_new, dt_done);
        if (fabs(f_new) < tolerance || fabs(t_new - t2) < tolerance_t) {
            return t_new;
        }
        if (f_new > 0) {
            t2 = t_new;
    ...
    error->warning(FLERR, "Maximum iterations reached in secant method.");
    return t2; 
}

/* ---------------------------------------------------------------------- */

double Update::secand_method(double* v, double* x, RotateLine& rl, double t1, double t2, double dt_done)
{
    const double tolerance = 1e-10 * MIN(domain->boxhi[0] - domain->boxlo[0],
        domain->boxhi[1] - domain->boxlo[1]); // need further test
    const int max_iter = 100; // need further test
    ...
    for (int i = 0; i < max_iter; ++i) {
        ...
        double t_new = t2 - f2 * (t2 - t1) / (f2 - f1);
        double f_new = caculate_yP(v, x, rl, t_new, dt_done);
        if (fabs(f_new) < tolerance || fabs(t_new - t2) < tolerance) {
            return t_new;
        }
        t1 = t2;    f1 = f2;
        t2 = t_new; f2 = f_new;
...
/* ----------------------------------------------------------------------
return collision time t (t>0), otherwise return -1.0;
*/

double Update::rotate_collide_time(double* v, double* x, RotateLine& rl, double dtmax, double dt_done, double* point)
{
    //double t = secand_method(v, x, rl, 0, dt, dt_done);
    double t = secand_dichotomy_method(v, x, rl, 0, dtmax, dt_done);
    if (!(t > 0.0 && t <= dtmax)) return -1.0;
    double xP = caculate_xP(v, x, rl, t, dt_done);
    if ((xP > rl.x_A && xP > rl.x_B)
        || (xP < rl.x_A && xP < rl.x_B)) return -1.0;
    point[0] = x[0] + v[0] * t;
    point[1] = x[1] + v[1] * t;
    double theta_t = rl.theta0 + ms.rotate_omega * (t + dt_done);
    //if ((!rl.reverse_flag && (v[0] * cos(theta_t) + v[1] * cos(theta_t) > 0)) || 
    //    (rl.reverse_flag && (v[0] * cos(theta_t) + v[1] * cos(theta_t) < 0))) return -1.0;
    //printf("\n rotate_collide_time: x: %g %g %g || v: %g %g %g || rl.theta0:%g r:%g x_A:%g x_B:%g || dtmax: %g \n",
    //    x[0], x[1], x[2], v[0], v[1], v[2], rl.theta0, rl.r, rl.x_A, rl.x_B, dtmax);
    //printf("  collision point: %g %g, xP: %g \n", point[0], point[1], xP);
    return t;
}
...
template < int DIM, int SURF > void Update::move()
{
  ...
  if (ms.flag && DIM == 2) {
      delete[] rotateLine;
      rotateLine = new RotateLine[surf->nsurf];
      for (bigint i = 0; i < surf->nsurf; ++i) {
          caculate_rl(surf->lines[i].p1, surf->lines[i].p2, rotateLine[i]);
      }
  }
    ...
    for (int i = pstart; i < pstop; i++) {
	  ...
	  nscheck_one += nsurf;
          ...
          if (nsurf || ms.flag) {
            ...
            if (ms.flag && DIM == 2) {
                for (m = 0; m < surf->nsurf; m++) {
                    isurf = m;
                    //printf("isurf = %d", isurf);
                    //if (DIM > 1) {
                    ...
                    t = rotate_collide_time(v, x, rotateLine[isurf], dtremain * frac, dt-dtremain, xc);
                    //printf("after rotate_collide_time\n");
                    //printf("print addrs: %p %p %p %p %p %p %p %p\n",&t,&hitflag,&param,&side,&cflag,&minparam,&minsurf,minxc);
                ...
                //if (cflag) {
                //    printf("Before MoveSurf collision: ipart = %d, v = %g %g %g, x = %g %g %g, isurf = %d", 
                //        ipart->id, ipart->v[0], ipart->v[1], ipart->v[2], 
                //        ipart->x[0], ipart->x[1], ipart->x[2], minsurf);
                //}
            ...
            if (cflag) {
              ...
              if (DIM != 3)
              {
                  if (!ms.flag || DIM != 2){
                      ...
                      const double* o = ms.rotate_o;
                      const double omega = ms.rotate_omega;
                      double dx = sqrt((minxc[0] - o[0]) * (minxc[0] - o[0]) 
                          + (minxc[1] - o[1]) * (minxc[1] - o[1]));
                      double theta_ct = atan2(minxc[1] - o[1], (minxc[0] - o[0]));
                      //double theta_t = rotateLine[isurf].theta0 + omega * t;
                      double v_wall[2]{ -omega * dx * sin(theta_ct), omega * dx * cos(theta_ct) };
                      ipart->v[0] -= v_wall[0];
                      ipart->v[1] -= v_wall[1];
                      double norm[3]{};
                      double tsum = dt-dtremain;
                      norm[0] = line->norm[0] * cos(omega * tsum) - line->norm[1] * sin(omega * tsum);
                      norm[1] = line->norm[0] * sin(omega * tsum) + line->norm[1] * cos(omega * tsum);
                      norm[2] = 0.0;
                      jpart = surf->sc[line->isc]->
                          collide(ipart, norm, dtremain, line->isr, reaction);
                      ipart->v[0] += v_wall[0];
                      ipart->v[1] += v_wall[1];
                      //printf("After MoveSurf collision: ipart = %d, v = %g %g %g, x = %g %g %g, isurf = %d",
                      //        ipart->id, ipart->v[0], ipart->v[1], ipart->v[2],
                      //        ipart->x[0], ipart->x[1], ipart->x[2], minsurf);
                  }
              }

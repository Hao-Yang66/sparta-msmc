...
const CommMacro* SPARTA_NS::GridCommMacro::interpolation_2d()
{
    ...
    if (intercell == icell) ++count_originInter;
        ...
        int jcell = static_cast<int>(intercell - grid->cells);
        const bool valid_temp = intercell->macro.Temp > 0.0;
        const bool owned_cell = (jcell >= 0 && jcell < grid->nlocal);
        const bool relax_ok = owned_cell ? grid->cinfo[jcell].macro.do_relaxation : 1;
        if (!relax_ok || !valid_temp) {
            ++count_warningInter;
            intercell = icell;
        }
        else {

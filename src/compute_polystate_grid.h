...
ComputeStyle(polystate/grid,ComputePolyStateGrid)

#else

#ifndef SPARTA_COMPUTE_POLYSTATE_GRID_H
#define SPARTA_COMPUTE_POLYSTATE_GRID_H

#include "compute.h"

namespace SPARTA_NS {

class ComputePolyStateGrid : public Compute {
 public:
  ComputePolyStateGrid(class SPARTA *, int, char **);
  ~ComputePolyStateGrid();
  void init();
 ...
 protected:
  ...
  int *group2species;        // singleton species index for each mixture group
...
E: Compute polystate/grid mixture ID does not exist

Self-explanatory.

E: Number of groups in compute polystate/grid mixture has changed

This mixture property cannot be changed after this compute command is
issued.

E: Invalid call to ComputePolyStateGrid::post_process_grid()
